  ...
  function copyEmail() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(email).then(showCopyToast).catch(secours);
    } else secours();
  }
...
/* ===== Activation au premier appui, sur tout le site =====
   Dans certains navigateurs integres (celui de l'application Google), le click
   n'est pas emis au 1er appui : le doigt est alors consomme par l'emulation
   souris. Mesure sur enregistrement : le surlignage natif apparait bien sur le
   bouton, la page ne change en rien, et aucune action ne part.
   On declenche donc l'action sur touchend, sans dependre du click synthetique.

   preventDefault() supprime le click synthetique, puis on emet un vrai click sur
   l'element REELLEMENT touche (et non sur son parent), pour que event.target
   reste juste — closeModalOutside() en depend pour ne pas fermer la fenetre
   quand on touche a l'interieur.

   Exclusions volontaires :
   - champs de saisie (input, select, textarea, label) : preventDefault
     empecherait le focus et l'ouverture du clavier ;
   - liens target="_blank" : l'ouverture d'onglet doit rester le geste natif ;
   - liens tel: et mailto:, et les cartes "copier l'e-mail" : voir ci-dessous ;
   - glissements (>12 px) et appuis longs (>700 ms) : ce sont des defilements
     ou des selections de texte, on ne s'en mele pas.
   Rien de tout ceci ne s'active a la souris.

   ===== Pourquoi tel:, mailto: et copyEmail sont exclus =====
   Le click emis par n.click() est un click de script : isTrusted vaut false.
   Pour une navigation interne (les cartes de service) le navigateur l'accepte,
   d'ou le correctif qui a fonctionne sur ces cartes. Mais passer la main a une
   autre application (composeur telephonique, client mail) et ecrire dans le
   presse-papiers sont des actions privilegiees : le navigateur integre de
   l'application Google exige pour cela un vrai geste utilisateur, et refuse
   donc en silence le click de script.
   Resultat : au 1er appui, le click synthetique est refuse, et le vrai click
   du navigateur - le seul qui aurait marche - etait annule par la garde
   anti-doublon ci-dessous. Au 2e appui le navigateur considere la page comme
   deja activee par l'utilisateur et laisse passer : d'ou le double appui.
   Ces elements sont donc sortis du dispositif. cible() renvoyant null pour eux,
   ni preventDefault(), ni click() de script, ni garde anti-doublon ne les
   touchent : le navigateur traite l'appui lui-meme, des le premier. */
(function () {
  if (!('ontouchstart' in window) && !(navigator.maxTouchPoints > 0)) return;

  var CIBLES = 'a[href], button, [onclick], [role="button"]';
  var EXCLUS = 'input, select, textarea, label, [contenteditable], [target="_blank"],' +
               'a[href^="tel:"], a[href^="mailto:"], [onclick*="copyEmail"]';

  var x0 = 0, y0 = 0, t0 = 0, candidat = null;
  var emissionEnCours = false, dernier = null, dernierT = 0;

  function cible(n) {
    if (!n || !n.closest) return null;
    if (n.closest(EXCLUS)) return null;
    return n.closest(CIBLES);
  }

  document.addEventListener('touchstart', function (e) {
    candidat = null;
    if (e.touches.length !== 1) return;
    candidat = cible(e.target);
    if (!candidat) return;
    x0 = e.touches[0].clientX; y0 = e.touches[0].clientY; t0 = Date.now();
  }, { passive: true });

  document.addEventListener('touchend', function (e) {
    var c = candidat; candidat = null;
    if (!c || e.changedTouches.length !== 1) return;
    var t = e.changedTouches[0];
    if (Math.abs(t.clientX - x0) > 12 || Math.abs(t.clientY - y0) > 12) return;
    if (Date.now() - t0 > 700) return;

    // emettre le click sur l'element touche, pas sur l'ancetre interactif
    var n = e.target;
    while (n && typeof n.click !== 'function') n = n.parentElement;
    if (!n) n = c;

    e.preventDefault();
    emissionEnCours = true;
    try { n.click(); } finally { emissionEnCours = false; }
    dernier = c; dernierT = Date.now();
  }, { passive: false });

  // Si le navigateur emet malgre tout son propre click, on l'ignore.
  document.addEventListener('click', function (e) {
    if (emissionEnCours) return;
    var c = cible(e.target);
    if (c && c === dernier && Date.now() - dernierT < 700) {
